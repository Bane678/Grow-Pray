...
export function useGardenState(xp: number, coins: number, onSpendCoins?: (amount: number) => void, userMaxGridSize?: number): GardenStateResult {
  ...
  /**
   * Remove several planted trees at once.
   *
   * One state update and one write for the whole batch, rather than looping
   * removePlantedTree - that would re-derive `gardenData` from a stale closure
   * on every iteration and drop all but the last removal.
   *
